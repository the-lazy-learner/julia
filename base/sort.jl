...
function _sort!(v::AbstractVector, a::IEEEFloatOptimization, o::Ordering;
    ...
    elseif eltype(v) <: Integer && o isa Perm{DirectOrdering} && is_concrete_IEEEFloat(eltype(o.data))
        ...
        (_, j), _ = send_to_end!(i -> after_zero(o.order, @inbounds ip[i]), v, Forward; lo, hi)
...
function _sort!(v::AbstractVector, a::RadixSort, o::DirectOrdering;
                ...
                U = UIntMappable(eltype(v), o), buffer=nothing, kw...)
...
function _sort!(v::AbstractVector, a::PartialQuickSort, o::Ordering;
                lo=firstindex(v), hi=lastindex(v), buffer=similar(v),
                t=reinterpret(eltype(v), checkbounds(Bool, buffer, lo:hi) ? buffer : resize!(buffer, length(v))),
    ...
    while lo < hi && hi - lo > SMALL_THRESHOLD
        ...
        elseif j-lo < hi-j
            ...
            _sort!(v, a, o; lo, hi=j-1, buffer, t, swap, rev, kw...)
            lo = j+1
            rev = !rev
        else # Sort the higher part recursively
            _sort!(v, a, o; lo=j+1, hi, buffer, t, swap, rev=!rev, kw...)
            hi = j-1
        end
    end
    hi < lo && return v
    ...
    rev && reverse!(v, lo, hi)
    _sort!(v, a.next, o; lo, hi, buffer, t, kw...)
end
...
function sort!(v::AbstractVector{T};
    ...
    if buffer === nothing # TODO: reduce redundancy
        _sort!(v, alg, ord(lt,by,rev,order))
    else
        _sort!(v, alg, ord(lt,by,rev,order); buffer)
    end
end
...
@noinline function sort_chunks!(Av, n, alg, order, buffer)
    ...
    for lo = first(inds):n:last(inds)
        _sort!(Av, alg, order; lo, hi=lo+n-1, buffer)
    end
    Av
end
# TODO: reduce redundancy
@noinline function sort_chunks!(Av, n, alg, order, buffer::Nothing)
    inds = LinearIndices(Av)
    for lo = first(inds):n:last(inds)
...
function _sort!(v::AbstractVector, a::MergeSortAlg, o::Ordering;
                lo=firstindex(v), hi=lastindex(v), buffer=nothing)
    @inbounds if lo < hi
        ...
        _sort!(v, a, o; lo, hi=m, buffer=t)
        _sort!(v, a, o; lo=m+1, hi, buffer=t)
